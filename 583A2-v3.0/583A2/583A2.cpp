...
int main(int argc, const char * argv[])
{
    ...
    cout<<endl<<"dtime="<<duration<<endl<<endl;
...
void sim_annealing(Grids **mygrids,PLC &plcnet,int init_cost,int gridsize)
{
    ...
    for(k=0;k<50;k++)
    {
        ...
        plcnet.swap_cells(mygrids,i,j,old_cost,new_cost);
    ...
    while(1)
    {
        ...
        for(k=0;k<move_num;k++)//O(wire_num*cell_num*iter_num)
        {
            ...
            if(new_cost>old_cost)
            {
                ...
                {
                    plcnet.swap_cells(mygrids,i,j,old_cost,new_cost); //re-swap
                    num_acc--;
                }
            }
        }
        ...
        R_acc = (double)num_acc/(double)move_num;
        if(R_acc>0.96) {alpha = 0.5;}
        else if(R_acc>0.8) {alpha = 0.9;}
        else if(R_acc>0.15) {alpha = 0.95;}
        else {alpha = 0.8;}
        T = T*alpha;
        cout<<"R_acc="<<R_acc<<" alpha="<<alpha<<" T="<<T<<endl;
        cout<<"count="<<count<<" cost="<<new_cost<<endl<<endl;
        ...
        //cout<<"D_limit.x="<<D_limit.x<<" D_limit.y="<<D_limit.y<<endl;
    }
}
